...
BACKWARD
{
    ...
    ivec2 dim = ivec2(parameters.grid.shape[1] - 1, parameters.grid.shape[0] - 1);
    vec2 grid_coord = ncoord * vec2(dim);
    vec2 alpha = fract(grid_coord);
    ivec2 p = clamp(ivec2(grid_coord), ivec2(0), dim - 1);
    float_ptr g00 = param_grad_buffer(parameters.grid, p + ivec2(0,0));
    float_ptr g01 = param_grad_buffer(parameters.grid, p + ivec2(1,0));
    float_ptr g10 = param_grad_buffer(parameters.grid, p + ivec2(0,1));
    float_ptr g11 = param_grad_buffer(parameters.grid, p + ivec2(1,1));
